...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (!m || this.processedMessages.has(m.key.id)) {
            return;
        }
        this.processedMessages.set(m.key.id, now);
        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin.before === 'function') {
                if (await plugin.before.call(this, m, { match, conn: this, participants, groupMetadata, user, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems, chatUpdate, __dirname: ___dirname, __filename })) {
                    continue;
                }
            }
            ...
            if (match) {
                ...
                const extra = {
                    match, usedPrefix, noPrefix, args, command, text, conn: this, participants, groupMetadata, user, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems, chatUpdate, __dirname: ___dirname, __filename
                };
