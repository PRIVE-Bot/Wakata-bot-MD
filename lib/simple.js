...
export function serialize() {
    ...
    user: {
        get() {
            return (this.key && this.key.fromMe ?
                (conn.user.id.includes(':') ? conn.user.id : conn.user.id.split('@')[0]) :
                (this.participant || this.key.participant || this.key.remoteJid || '')).replace(/@.+/, '')
        },
    },
