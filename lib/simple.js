...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    return Object.defineProperties(conn, {
        ...
        sendSylph: { async value(jid, text = '', buffer, title, body, url, quoted, options) { if (buffer) try { buffer = (await conn.getFile(buffer)).data } catch { }; return conn.relayMessage(jid, (await generateWAMessageFromContent(jid, { extendedTextMessage: { text, contextInfo: { externalAdReply: { title, body, thumbnail: buffer, sourceUrl: url }, mentionedJid: await conn.parseMention(text) } } }, { quoted })).message, { messageId: (await generateWAMessageFromContent(jid, { extendedTextMessage: { text, contextInfo: { externalAdReply: { title, body, thumbnail: buffer, sourceUrl: url }, mentionedJid: await conn.parseMention(text) } } }, { quoted })).key.id }); } },
        getFile: {
...
export function serialize() {
    if (!proto.WebMessageInfo?.prototype) return console.error("proto.WebMessageInfo.prototype no definido");
