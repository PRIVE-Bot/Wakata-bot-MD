...
import { setupErrorHandler } from './plugins/tools-arror.js'
...
/* let prefixjos = '#/.-'
global.prefix = global.prefijo ? new RegExp('^' + global.prefijo) : new RegExp('^[' + (opts['prefix'] || prefixjos).replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&') + ']')
*/

let prefixjos = '#/.-'
global.prefix = global.prefijo
  ? new RegExp('(?:^|\\S)' + global.prefijo.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&'))
  : new RegExp('(?:^|\\S)[' + (opts['prefix'] || prefixjos).replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&') + ']')
...
global.jadi = global.jadi || 'bot'
const pathJadi = join(__dirname, global.jadi)

if (!existsSync(pathJadi)) {
mkdirSync(pathJadi, { recursive: true })
