...
var handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if (!searchA.length) return conn.reply(m.chat, '❌ No se encontraron resultados.', m, fake)
...
var handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    let searchA = await search(text)
    if (!searchA.length) return conn.sendMessage(m.chat, { text: '❌ No se encontraron resultados.', edit: msg.key })
...
┗━━━━━━━━━━━━━━━━⍰`
    ...
    // editar mensaje y añadir icono como thumbnail
    await conn.sendMessage(m.chat, {
