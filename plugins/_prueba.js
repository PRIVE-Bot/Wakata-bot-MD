...
let sugerirHandler = async (m, { conn, text, usedPrefix }) => {
  ...
  // Reemplaza 'CREADOR_ID@s.whatsapp.net' y 'STAFF_GROUP_ID' por los IDs correspondientes.
  await conn.reply('CREADOR_ID@s.whatsapp.net', teks, m, { mentions: conn.parseMention(teks) })
  await conn.reply('STAFF_GROUP_ID', teks, m, { mentions: conn.parseMention(teks) })
