...
let sugerirHandler = async (m, { conn, text, usedPrefix }) => {
  ...
  if (nuevoComando.length < 1) {
    return conn.reply(m.chat, `❗️ El nombre del comando es muy corto.`, m, rcanal);
  }

  if (descripcion.length < 10) {
  return conn.reply(m.chat, `❗️ La descripción debe tener al menos 10 caracteres.`, m, rcanal);
}

if (descripcion.length > 1000) {
  return conn.reply(m.chat, `❗️ La descripción debe tener máximo 1000 caracteres.`, m, rcanal);
}
  ...
  let staffGroup = '120363419125812808@g.us';
