...
const shapes = {
  ...
  st: (ctx, img) => { const spikes=5; const outer=256; const inner=100; let rot=Math.PI/2*3; let x=256; let y=256; let step=Math.PI/spikes; ctx.beginPath(); ctx.moveTo(x,y-outer); for(let i=0;i<spikes;i++){ctx.lineTo(x+Math.cos(rot)*outer,y+Math.sin(rot)*outer); rot+=step; ctx.lineTo(x+Math.cos(rot)*inner,y+Math.sin(rot)*inner); rot+=step} ctx.closePath(); ctx.clip(); ctx.drawImage(img,0,0,512,512) },
  he: (ctx,img)=>{ const r=256; const cx=256; const cy=256; ctx.beginPath(); for(let i=0;i<6;i++){ const a=Math.PI/3*i-Math.PI/6; const x=cx+r*Math.cos(a); const y=cy+r*Math.sin(a); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y)} ctx.closePath(); ctx.clip(); ctx.drawImage(img,0,0,512,512) },
  ...
  try {
    ...
    if (/video|gif/.test(mime)) {
      ...
      await new Promise((resolve,reject)=>{
        ...
        .toFormat('webp').save(tempOut).on('end',resolve).on('error',reject)
      })
      ...
      if(/webp/.test(mime)){ const tempP=tmp('png'); fs.writeFileSync(tempP,img); await new Promise((r,e)=>ffmpeg(tempP).toFormat('png').save(tempP+'.png').on('end',r).on('error',e)); img=fs.readFileSync(tempP+'.png'); fs.unlinkSync(tempP); fs.unlinkSync(tempP+'.png') }
