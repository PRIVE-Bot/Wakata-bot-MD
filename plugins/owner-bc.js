...
const handler = async (m, {conn, command, participants, usedPrefix, text}) => {
  if (!text) return conn.reply(m.chat, `${emoji} Te faltó el texto que quieres transmitir a todos los chats.`, m, fake);
  ...
  await conn.reply(m.chat, `${emoji} *El texto se está enviando a todos los chats*`, m, fake);
