...
async function handler(m, { text, conn, usedPrefix, command }) {
  if (!text) return conn.reply(m.chat, `Usa: ${usedPrefix + command} <texto> [delay=ms repeat=n]`, m)
  ...
  if (!hasColor) {
    try {
      ...
      const interactiveMessage = {
        ...
        header: { title: 'BRAT GIF Color', hasMediaAttachment: true, imageMessage: media.imageMessage },
  ...
  try { await conn.reply(m.chat, '⏳ Generando sticker animado BRAT...', m) } catch {}
  const cleaned = text.trim()
  try {
    const { buffer, finalFontSize, finalLayout, delay, repeat, frames, opts, lastFramePNG } = await generateBratGif(cleaned)
    ...
    try {
      const animSticker = new Sticker(buffer, { pack, author, type: 'crop', quality: 50, id: 'brat-gif-anim' })
      stickerBuffer = await animSticker.toBuffer()
      if (stickerBuffer && stickerBuffer.length) animatedOk = true
    } catch {}
    if (!animatedOk) {
      // Fallback a sticker estático usando último frame
      try {
        const staticSticker = new Sticker(lastFramePNG, { pack, author, type: 'full', quality: 90, id: 'brat-gif-static' })
